...
const ManageCars = () => {
  ...
  const toggleAvailability = async(carId)=>{
    try {
      const {data} = await axios.post('/api/owner/toggle-car', {carId})  //carId is being sent at api endpoint
      if(data.success){ 
  ...
  const deleteCar = async(carId)=>{
    try {
      ...
      const {data} = await axios.post('/api/owner/delete-car', {carId})  //carId is being sent at api endpoint
      if(data.success){ 
        toast.success(data.message)
        fetchOwnerCar()
      }else{
  ...
  return (
    <div className='px-4 pt-10 md:px-10 w-full'>
      <Title title='Manage Cars' subTitle='View all listed cars, update their datails or remove hem from the booking platform.'/>
        ...
        <table className='w-full border-collapse text-left text-sm text-gray-600'>
          <thead className='text-gray-500'>
            <tr >
              ...
              <th className='p-3 font-medium max-md:hidden'>Catgory</th>
